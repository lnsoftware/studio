...
package com.imxiaomai.convenience.store.scan.zxing.camera;
...
//			camera = Camera.open();
	...
	public Rect getFramingRect() {
        try {
            ...
            //Log.d(TAG, "CameraManager getFramingRect screenResolution = " + screenResolution);
            if (framingRect == null) {
                if (camera == null) {
                    return null;
                }
                ...
                if (null != screenResolution) {
                    widthScreen = screenResolution.x;
                    heightScreen = screenResolution.y;
                } else {
                ...
                // 扫描框距离顶部和左边的距离
                int leftOffset = (widthScreen - width) / 2;
                int topOffset = (heightScreen - height) / 3;
